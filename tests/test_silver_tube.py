...
WTV_VIDEO_DEF = VideoDefinition(
    resolution=Resolution.STANDARD_DEF,
...
class SilverTubeTestCase(unittest.TestCase):
    def setUp(self):
        ...
        config["ccextractor"] = {"executable": ccextractor(), "run.if.missing": True}
        comskip_ini = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "comskip.ini"
        )
        config["comskip"] = {
            "executable": comskip(),
        ...
        self.dirs.extend(
            [self.tv_in, self.commercial_in, self.srt_in, self.temp_dir, self.out_dir]
        )
    ...
    def test_config_file_created(self):
        ...
        self.assertEqual(
            ccextractor(), parser.get("ccextractor", "executable", fallback=None)
        )
    ...
    def test_basic(self, tvdb_mock):
        ...
        create_test_video(
            length=10, video_def=WTV_VIDEO_DEF, output_file=input_file, metadata=meta
        )
