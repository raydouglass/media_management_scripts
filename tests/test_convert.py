...
class ConvertTestCase(unittest.TestCase):
    ...
    def test_hevc_convert(self):
        config = convert_config_from_ns({'video_codec': VideoCodec.H265.ffmpeg_encoder_name})
    ...
    def test_mpeg2_convert(self):
        config = convert_config_from_ns({'video_codec': VideoCodec.MPEG2.ffmpeg_encoder_name})
        with create_test_video(length=3) as file, NamedTemporaryFile(suffix='.mkv') as output:
            convert_with_config(file.name, output.name, config, overwrite=True)
            metadata = extract_metadata(output.name)
    ...
    def test_audio_convert(self):
        config = convert_config_from_ns({'audio_codec': AudioCodec.AC3.ffmpeg_codec_name})
        with create_test_video(length=3) as file, NamedTemporaryFile(suffix='.mkv') as output:
            convert_with_config(file.name, output.name, config, overwrite=True)
            metadata = extract_metadata(output.name)
    ...
    def test_multiple_audio(self):
        config = convert_config_from_ns({})
        with create_test_video(length=3, audio_defs=[AudioDefition(codec=AudioCodec.AC3),
                                                     AudioDefition(codec=AudioCodec.AC3)]) as file, \
                NamedTemporaryFile(suffix='.mkv') as output:
            convert_with_config(file.name, output.name, config, overwrite=True)
            metadata = extract_metadata(output.name)
            self.assertEqual(2, len(metadata.audio_streams))
            self.assertEqual(metadata.audio_streams[0].codec, AudioCodec.AAC.ffmpeg_codec_name)
            self.assertEqual(metadata.audio_streams[1].codec, AudioCodec.AAC.ffmpeg_codec_name)

    def test_multiple_audio_mappings(self):
        config = convert_config_from_ns({'audio_codec': 'copy'})
        with create_test_video(length=3, audio_defs=[AudioDefition(codec=AudioCodec.AAC),
                                                     AudioDefition(codec=AudioCodec.AC3)]) as file, \
                NamedTemporaryFile(suffix='.mkv') as output:
            convert_with_config(file.name, output.name, config, mappings=[2], overwrite=True)
            metadata = extract_metadata(output.name)
    ...
    def test_multiple_audio_mappings_str(self):
        ...
        with create_test_video(length=3, audio_defs=[AudioDefition(codec=AudioCodec.AAC),
                                                     AudioDefition(codec=AudioCodec.AC3)]) as file, \
                NamedTemporaryFile(suffix='.mkv') as output:
            convert_with_config(file.name, output.name, config, mappings=['a:1'], overwrite=True)
            metadata = extract_metadata(output.name)
            self.assertEqual(1, len(metadata.audio_streams))
            self.assertEqual(metadata.audio_streams[0].codec, AudioCodec.AC3.ffmpeg_codec_name)
