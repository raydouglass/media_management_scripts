...
class VideoMixin():
    def create(self, video_def, audio_def):
        self.count += 1
        create_test_video(1, video_def=video_def, audio_defs=[audio_def],
...
class SearchTestCase(unittest.TestCase, VideoMixin):
    ...
    def test_meta(self):
        create_test_video(1, video_def=H264_VIDEO_DEF, audio_defs=[AAC_STEREO_AUDIO_DEF],
                          output_file=os.path.join(self.tmpdir.name, 'test.mp4'), metadata={'title': 'whatever'})
        self.run_test(1, 'meta.title = whatever')
        self.run_test(1, 'meta.tags.title = whatever')
        create_test_video(1, video_def=H264_VIDEO_DEF, audio_defs=[AAC_STEREO_AUDIO_DEF],
                          output_file=os.path.join(self.tmpdir.name, 'test2.mp4'), metadata={'title': 'whatever two'})
