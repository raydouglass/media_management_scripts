from media_management_scripts.support.encoding import VideoCodec, VideoFileContainer, Resolution
from tests import create_test_video, VideoDefinition, assertAudioLength
...
MP4_VIDEO_DEF = VideoDefinition(Resolution.LOW_DEF, VideoCodec.H264, VideoFileContainer.MP4)
...
class ConcatMp4TestCase(unittest.TestCase):
    ...
    def test_3_files(self):
        with create_test_video(length=5, video_def=MP4_VIDEO_DEF) as first, \
                create_test_video(length=5, video_def=MP4_VIDEO_DEF) as second, \
                create_test_video(length=5, video_def=MP4_VIDEO_DEF) as third:
            with NamedTemporaryFile(suffix='.mp4') as output:
                concat_mp4(output.name, files=[first.name, second.name, third.name], overwrite=True)
                metadata = create_metadata_extractor().extract(output.name)
                self.assertEqual(1, len(metadata.video_streams))
